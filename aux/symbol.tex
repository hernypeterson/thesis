\DeclareAcronym{az1}{
  short = \ensuremath{\theta_1},
  long = paleo-azimuth,
  tag = sym,
  code = az1
}

\DeclareAcronym{sl1}{
  short = \ensuremath{\varphi_1},
  long = paleo-slope,
  tag = sym,
  code = sl1
}

\DeclareAcronym{az2}{
  short = \ensuremath{\theta_2},
  long = azimuth,
  tag = sym,
  code = az2
}

\DeclareAcronym{sl2}{
  short = \ensuremath{\varphi_2},
  long = slope,
  tag = sym,
  code = sl2
}

\DeclareAcronym{discord}{
  short = \ensuremath{\Delta\theta},
  long = discordance,
  tag = sym,
  code = disc
}

\DeclareAcronym{center}{
  short = \ensuremath{c},
  long = summit inferred from paleo-topography,
  tag = sym
}

\DeclareAcronym{bearing}{
  short = \ensuremath{\Theta},
  long = bearing from \acs{center},
  tag = sym,
  code = theta
}

\DeclareAcronym{dist}{
  short = \ensuremath{r},
  long = radial distance,
  tag = sym,
  code = dist
}

\DeclareAcronym{normal2}{
  short = \ensuremath{\vec{n}_2},
  long = pole to modern topographic surface plane,
  tag = sym
}

\DeclareAcronym{normal1}{
  short = \ensuremath{\vec{n}_1},
  long = pole to estimated paleo-surface plane,
  tag = sym
}

\DeclareAcronym{proj2}{
  short = \ensuremath{\Phi_2},
  long = slope of line intersecting the vertical \acs{bearing} plane and surface 2 (modern),
  tag = sym,
  code = proj2
}

\DeclareAcronym{proj1}{
  short = \ensuremath{\Phi_1},
  long = slope of line intersecting the vertical \acs{bearing} plane and surface 1 (paleo),
  tag = sym,
  code = proj1
}

\DeclareAcronym{tilt}{
  short = \ensuremath{\Delta\Phi},
  long = axisymmetric tilt,
  tag = sym,
  code = axtilt
}

\DeclareAcronym{tilt_min}{
  short = \ensuremath{\alpha},
  long = tilt,
  tag = sym,
  code = tilt
}

\DeclareAcronym{north}{
  short = \textbf{N},
  long = north,
  tag = sym
}

\DeclareAcronym{disp_a}{
  short = \ensuremath{U_A},
  long = displacement of vertex $A$ (proximal),
  tag = sym
}
\DeclareAcronym{disp_b}{
  short = \ensuremath{U_B},
  long = displacement of vertex $B$ (distal),
  tag = sym
}
\DeclareAcronym{ddisp}{
  short = \ensuremath{\Delta U},
  long = $\acs{disp_b} - \acs{disp_a}$,
  tag = sym
}
\DeclareAcronym{disp}{
  short = \ensuremath{U},
  long = displacement,
  tag = sym,
  code = disp
}
\DeclareAcronym{disp_z}{
  short = \ensuremath{U_z},
  long = vertical component of \acs{disp},
  tag = sym,
  code = disp[1]
}
\DeclareAcronym{disp_r}{
  short = \ensuremath{U_r},
  long = radial component of \acs{disp},
  tag = sym,
  code = disp[0]
}

\DeclareAcronym{ddisp_z}{
  short = \ensuremath{\Delta U_z},
  long = vertical component of \acs{ddisp},
  tag = sym
}
\DeclareAcronym{ddisp_r}{
  short = \ensuremath{\Delta U_r},
  long = radial component of \acs{ddisp},
  tag = sym
}

\DeclareAcronym{disp_z'}{
  short = \ensuremath{U_z'},
  long = derivative of \acs{ddisp_z} with respect to $r$,
  tag = sym
}
\DeclareAcronym{disp_r'}{
  short = \ensuremath{U_r'},
  long = derivative of \acs{ddisp_r} with respect to $r$,
  tag = sym
}

\DeclareAcronym{z1'}{
  short = \ensuremath{z_1'},
  long = slope (essentially),
  tag = sym
}
\DeclareAcronym{dr1}{
  short = \ensuremath{\Delta r_1},
  long = radial distance between $A$ and $B$,
  tag = sym
}
\DeclareAcronym{dz1}{
  short = \ensuremath{\Delta z_1},
  long = vertical distance between $A$ and $B$,
  tag = sym
}
\DeclareAcronym{DtT}{
  short = \ensuremath{DtT},
  long = depth to top of magma reservoir,
  tag = sym,
  code = DtT
}
\DeclareAcronym{Rr}{
  short = \ensuremath{R_r},
  long = magma reservoir radial (horizontal) radius,
  tag = sym,
  code = Rr
}
\DeclareAcronym{Rz}{
  short = \ensuremath{R_z},
  long = magma reservoir vertical radius,
  tag = sym,
  code = Rz
}
\DeclareAcronym{g}{
  short = \ensuremath{g},
  long = gravity of mars,
  tag = sym,
  code = GRAV
}
\DeclareAcronym{mult}{
  short = \ensuremath{mult},
  long = multiplier for over- or underpressure,
  tag = sym,
  code = mult
}
\DeclareAcronym{rhor}{
  short = \ensuremath{\rho_\text{rock}},
  long = rock density,
  tag = sym,
  code = rhor
}
\DeclareAcronym{dP}{
  short = \ensuremath{\Delta P},
  long = over- (positive) or under- (negative) pressure,
  tag = sym,
  code = dP
}
\DeclareAcronym{beta1}{
  short = \ensuremath{\beta_1},
  long = \ensuremath{\acs{az1}-\acs{bearing}},
  tag = sym,
  code = beta1
}
\DeclareAcronym{beta2}{
  short = \ensuremath{\beta_2},
  long = \ensuremath{\acs{az2}-\acs{bearing}},
  tag = sym,
  code = beta2
}