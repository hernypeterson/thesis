...
More importantly, even conditions which violate the analytical solution assumptions produce tilt functions of similar qualitative shapes. [SHOW THIS]. Therefore, tilt data resulting from unknown pressure change conditions can be fit to the right class of function. To see this, I proceed with the analytical tilt function derivation.  

Equation~\eqref{eq:tilt-from-model} in a half-space (flat; $\acs{dz1} = 0$),  reduces to:
\begin{equation}
    \acs{tilt} = 
