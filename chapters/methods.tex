...
\subsection{Paleo-Slope}

\begin{figure}
\begin{center}
    \input{figures/tilt-from-map.tex}%
    \caption[Tilt from Map]{One quadrant of an upper hemisphere stereographic projection. The point labeled $\left(\acs{az2},\acs{sl2}\right)$ represents the attitude of the modern topographic surface. The line labeled \acs{az1} represents the family of attitudes that could be represented by the surface based on the downhill lava flow direction. The figure is oriented so that the direction away from a chosen inflation center (\acs{bearing}) is up. In this geometry, \acs{beta1} and \acs{beta2} are defined as $\acs{az1}-\acs{bearing}$ and $\acs{az1}-\acs{bearing}$, respectively. The axisymmetric model condition imposes a tilt axis perpendicular to \acs{bearing}, which will translate any point in the projection vertically. Thus the tilt inferred is the rotation that brings some point on the \acs{az1} line vertically to meet $\left(\acs{az2},\acs{sl2}\right)$.}%
    \label{fig:tilt-from-map}
\end{center}
\end{figure}

While the modern topographic surface attitude is fully characterized by \acs{az2} and \acs{sl2}, only the \acf{az1} can be inferred directly from mapped surface features. The axisymmetric model imposes a circumferential tilt axis, that is, modelled tilt is always directly toward or away from the inflation center. Figure~\ref{fig:tilt-from-map} illustrates my method for calculating \acf{sl1} under this assumption. The horizontal segment labeled $\delta$ can be defined in two ways:
\begin{gather*}
...
\subsection{Axisymmetric Tilt from Mapped Discordance}
In Figure~\ref{fig:tilt-from-map}, the arrow which translates the \acs{az1} line onto $\left(\acs{az2},\acs{sl2}\right)$ is a segment of a small circle about the tilt axis. I determine the angle of tilt by essentially treating the $\left(\acs{az1},\acs{sl1}\right)$ and $\left(\acs{az2},\acs{sl2}\right)$ poles as apparent ``dips''\footnote{I use this very loosely because the apparent dip is a pole and not a physical linear feature; also, I allow these dips to take negative values rather than reversing the dip direction.} where the true ``dip'' direction is \acs{bearing}. Thus the true ``dips'' and their difference:
\begin{gather}
    \acs{proj1} = \arctan(\tan\acs{sl1}/\cos\acs{beta1})\\
    \acs{proj2} = \arctan(\tan\acs{sl2}/\cos\acs{beta2})\\
    \acs{tilt} = \acs{proj2} - \acs{proj1}.
\end{gather}
