...
\begin{figure}
    ...
    \caption[\Acl{tilt} from modelling]{Tilt from finite element displacement. A surface element is labelled ``Original Surface'' with slope \acs{proj1}. During the model, the two element vertices $A_1$ and $B_1$ are displaced via $\acs{disp_a}$ and $\acs{disp_b}$ to $A_2$ and $B_2$ respectively, bringing the element to the ``Deformed surface'' with slope \acs{proj2}. The horizontal (radial) and vertical components of the difference $\acs{disp_b} - \acs{disp_a} = \acs{ddisp}$ are labeled. Despite the difference in appearance, the quantity $\acs{proj2}-\acs{proj1}=\acs{tilt}$ (red) is the same quantity labelled in Figure~\ref{fig:tilt-from-map} (also red). The angles colored blue and green are closely related but not identical across the two figures (discussed in the body) and thus I do not explicitly analyze them as such.}%
