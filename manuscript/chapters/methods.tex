...
The \ac{CTX} mosaic was used to identify lava flows near the summit of \ac{OM}. Where possible, lobate flows were outlined and mapped as polygons. Flow centerlines were derived from these polygons, as shown in Figure~\ref{fig:flow}. Channels were mapped as lines where flow margins were not visible. Some channels were discontinuous; these features were inferred to be regions where channels formed lava tubes which have subsequently collapsed to form a chain of skylights. See Figure~\ref{fig:channel}.
...
\begin{figure}
    ...
    \caption[Surface attitude as a normal vector $\acs{normal}=(\acs{az},\acs{ze})$]{The attitude of a tilted surface (green) is defined relative to a flat surface (grey) by its \acf{az} and \acf{ze}. \acs{az} and \acs{ze} are also the spherical coordinates of a unique unit vector \acs{normal} which is normal (perpendicular) to the surface. In the subsequent sections, $\acs{normal}=(\acs{az},\acs{ze})$ will always represent the observed attitude at a sampled location, while $\acs{normal'}=(\acs{az'},\acs{ze'})$ will always represent the \emph{inferred} attitude of the same point when the lava flow was emplaced.}
    \label{fig:geometric-surface}
\end{figure}
The ``Surface Parameters'' tool was used in ArcGIS Pro to compute average \acf{ze} and \acf{az} across the entire study area. In both cases, a new raster is produced at the same spatial resolution and coverage as the original \ac{DEM}. To avoid capturing local topographic anomalies, these values are averaged\footnote{Technically, a quadratic surface is interpolated over the neighborhood region and dip and dip direction are computed at the center of this surface.} over a circular ``neighborhood'' with radius \qty{2}{\km}. To each sampling location identified in Section~\ref{sec:mapping} is assigned \acf{ze} and \acf{az} based on the value of the corresponding raster at that location. Figure~\ref{fig:geometric-deform-collected} shows a geometric view of the relevant data to be collected.

\section{Attitude Data Calculation}
\subsection{Minimum 3D Deformation}\label{3d-deform}
Figure~\ref{fig:geometric-deform-calculated} shows what it means to calculate \acs{ze'} such that the \acf{central-angle} between \acs{normal} and \acs{normal'} is minimized. The method for this calculation is presented below.
\begin{figure}
    \centering
    \input{figures/geometric-deform.tex}
    \caption{Geometric view of optimization method}\label{fig:geometric-deform}
\end{figure}
...
Figure~\ref{fig:geometric-radial} shows what is means to calculate the component of 3D deformation along the radial-axial plane. Comparison with Figure~\ref{fig:geometric-deform-calculated} reveals the geometric similarity between the two problems. Therefore, the same Equation~\ref{eq:ze'} derived in Section~\ref{3d-deform} will be used, with small modifications owing to a few important differences. First, there are two distinct blue central angles to be minimized, and thus two distinct red zenith angles to be calculated. The \acf{radial-deform} value to be calculated is the signed difference between the two zenith values, which correspond to the ``projections'' of \acs{normal} and \acs{normal'} onto the plane containing the $z-$ and $r-$axes. Also, the correction used in Equation~\eqref{eq:robust-ze'} is not desired here, since the vectors \acs{normal} and \acs{normal'} could just as easily project on either side of the $z-$axis.
\begin{equation}
