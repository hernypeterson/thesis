...
\begin{figure}
    ...
    \caption{Geometric view of optimization method}
    \label{geometric-optimize}
\end{figure}
...
\begin{equation}
    ...
    \end{bmatrix}.
\end{equation}
Thus the normal vectors \acs{point} and \acs{closest-point} can be expressed in radial-tangential form as \acs{change-basis}\acs{point} and \acs{change-basis}\acs{closest-point}, respectively. The component of 3D deformation (as minimized in Section~\ref{3d-deform}) in the radial-axial direction is therefore the angular distance between the projections of \acs{change-basis}\acs{point} and \acs{change-basis}\acs{closest-point} in the $r-z$ plane. We already have $z=\cos\ac{ze}$ from Equation~\ref{spherical-to-cartesian}. For $r$, we need just the first row of \acs{change-basis}:
\begin{equation}
    r=
    \begin{bmatrix}
        \cos\acs{THETA} & \sin\acs{THETA} & 0
