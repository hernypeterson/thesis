...
\begin{figure}
    ...
    \caption[Surface attitude as normal vector $\acs{normal}=(\acs{az},\acs{ze})$]{The attitude of a tilted surface (green) is defined relative to horizontal (grey) by its \acf{az} and \acf{ze}. \acs{az} and \acs{ze} are also the spherical coordinates of a unique unit vector \acs{normal} which is normal (perpendicular) to the surface. In the subsequent sections, $\acs{normal}=(\acs{az},\acs{ze})$ will always represent the observed attitude at a sampled location, while $\acs{normal'}=(\acs{az'},\acs{ze'})$ will always represent the \emph{inferred} attitude of the same point when the lava flow was emplaced. Note that azimuth angles always increase clockwise from due \acf{north} as shown, following the geographic convention.}
...
\begin{figure}
    ...
    \caption{Geometric view of \acl{ze'} calculation}\label{fig:geometric-deform}
\end{figure}
